...
| Term | Disposition | Interpretation |
|---|---|---|
| `sh:targetClass` (§3.1.3.2) | Supported-with-interpretation | Lowers to the SHACL-instance path `rdf:type/rdfs:subClassOf*` — subclass-typed instances included ([ADR-0025](adr/0025-relationship-targeting-model.md)); the closure reads the queried graphs only (Core §6.3's shapes-graph lookup is the ADR-0016 deviation); non-IRI values reject; a class targeted by two shapes rejects at registry build |
...
| Term | Disposition | Interpretation |
|---|---|---|
| `sh:class` (§7.1.1) | Supported-with-interpretation | Types the binding while `sh:node` names the target (ADR-0025): one or more IRIs emit one SHACL-instance typing path each (conjunction); alone, a sole IRI resolves the target shape (synthetic protected shape with a warning when untargeted); multiple values without `sh:node` reject — the intersection names no shape; literal values reject as ill-formed |
| `sh:class` list form (1.2 union) | Rejected-loudly | A loud placeholder until polymorphic relationships land ([ADR-0026](adr/0026-polymorphic-relationships.md)) — alone the list is a union of target classes, beside `sh:node` a binding union; neither lowers today, and the empty list (the vacuous union) rejects with them; the list form is consumed inside `shnex:filterShape` (§2) |
| `sh:node` on property shapes (§7.8.1) | Supported-with-interpretation | Names the target — the linked shape is the GraphQL type (ADR-0025); a class-indexed target auto-types the binding (its class's SHACL-instance path; explicit `sh:class` values replace the implicit), a class-less target emits no typing; multiple values reject (conjoined targets select no single type), blank-node values reject (inline node shapes — matching the node-shape host, above), and literal values reject as ill-formed; unknown IRI targets raise at schema build (registry), not parse |
...
| `sh:and` (§7.7.2) | Supported-with-interpretation | Class-conjunction slice only: members whose sole constraint is `sh:class` conjoin into the binding classes exactly as repeated `sh:class` values; any other member mix warns once and is inert — the operator's other uses are not consumed |
...
| The remaining §7 components | Recognised-and-inert | Uniform boundary: never read at shape level — no warning, no schema or query effect. Covers `sh:nodeKind`, `sh:pattern`, the string/numeric range groups, the property-pair group, `sh:languageIn`, `sh:uniqueLang`, `sh:closed`, `sh:hasValue`, the logical group (`sh:not`/`sh:xone`), the 1.2 list group, `sh:qualifiedValue*`, `sh:someValue`, `sh:reifierShape`, `sh:rootClass`, `sh:uniqueValuesFor`. Exception: inside `shnex:filterShape` a lowerable subset is consumed and everything else rejects loudly there (§2) |
