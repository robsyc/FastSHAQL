...
- **Root field names should honor `sh:codeIdentifier` as-is** — `core/schema/build.py::root_field_name` currently applies `graphql_type_name.lower() + 's'`, mangling camelCase identifiers (`MedicationAdministration` → `medicationadministrations`). Identifier validation itself is settled (parse-time §8.4 grammar check in `read_code_identifier`) — renaming root fields is a breaking change to the public query surface. *(Pre-impl: survey naming in other GraphQL-over-RDF generators.)*
