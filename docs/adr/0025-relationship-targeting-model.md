...
| `sh:node S` + `sh:class C` (one or more) | S | explicit classes **replace** the implicit; one path each — conjunction |
...
**Every rejected combination teaches the model** — error messages name the shape, the field, and the remedy ("multiple conjoined classes intersect but name no shape — add `sh:node` to select the target").

**Conjunction also arrives as `sh:and`.** Members whose sole constraint is `sh:class` conjoin into the binding classes (`sh:and` §7.7.2 conjoins exactly as repeated `sh:class` §3.1.1); any other member mix warns and stays wholly inert — consuming the class slice alone would loosen the constraint.
