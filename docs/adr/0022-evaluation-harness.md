...
- Adapters run stores in **stock configuration**: knobs that would converge a store to the harness's union contract (e.g. Oxigraph's `--union-default-graph`) stay deliberately unset, so divergence entries report default-deployment behavior. Fuseki's is the structural case — Jena's union excludes the stored unnamed graph, so no knob reaches the contract (ADR-0011)
