...
- `just mutate` gates on the committed mutation-score floor (`mutmut-floor.json`); the run is incremental from the cached `mutants/` directory. mutmut 3.x's copy step never deletes files removed from the source, so a stale `mutants/` tree can poison a fresh run — `just mutate-clean` is the complete reset. Test-file changes invalidate every mutant's cached status, so after adding tests only a full run yields truthful stats — a name-filtered `mutmut run` re-tests just its scope and leaves the rest uncounted.
