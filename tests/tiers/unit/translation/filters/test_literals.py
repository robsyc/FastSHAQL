...
def test_string_value_respects_declared_datatype() -> None:
    ...
    literal = value_to_literal(node, XSD.date)
    assert literal == Literal("2020-01-01", datatype=XSD.date)
    assert literal.datatype == XSD.date


def test_numeric_and_boolean_values_respect_declared_datatype() -> None:
    assert value_to_literal(IntValueNode(value="42"), XSD.date).datatype == XSD.date
    assert value_to_literal(FloatValueNode(value="1.5"), XSD.date).datatype == XSD.date
    assert value_to_literal(BooleanValueNode(value="true"), XSD.date).datatype == (
        XSD.date
    )
...
def test_value_defaults_without_declared_datatype() -> None:
    ...
    assert value_to_literal(IntValueNode(value="42"), None).datatype == XSD.integer
    assert value_to_literal(FloatValueNode(value="1.5"), None).datatype == XSD.float
    assert value_to_literal(BooleanValueNode(value="true"), None).datatype == (
        XSD.boolean
    )
